...
def JingyuanTingyunTopazLuocha(config):
    ...
    JingYuanCharacter = JingYuan(RelicStats(mainstats = ['ATK.percent', 'SPD.flat', 'CR', 'DMG.lightning'],
                            ...
                            relicsetone = GrandDuke2pc(), relicsettwo = GrandDuke4pc(followupStacks=6.5,stacks=8.0,uptime=1.0), planarset = InertSalsotto(stack=2),
                            **config)
    ...
    # Broken Keel Buffs
    for character in [JingYuanCharacter, TingyunCharacter, TopazCharacter]:
        character.addStat('CD',description='Broken Keel from Luocha',amount=0.1)
    for character in [JingYuanCharacter, LuochaCharacter, TopazCharacter]:
