...
def FireflyTrailblazerRuanMeiGallagher(config):
    ...
    FireflyCharacter = Firefly(RelicStats(mainstats = ['ATK.percent', 'SPD.flat', 'CR', 'BreakEffect'],
                                    substats = {'CD': 12, 'CR': 8, 'BreakEffect': 5, 'SPD.flat': 3}),
    ...
    TrailblazerCharacter = ImaginaryTrailblazer(RelicStats(mainstats = ['DMG.fire', 'SPD.flat', 'CR', 'ATK.percent'],
                                    substats = {'CR': 7, 'CD': 12, 'ATK.percent': 3, 'SPD.flat': 4}),
                                    lightcone = Swordplay(**config),
                                    relicsetone = ThiefOfShootingMeteor2pc(), relicsettwo = ThiefOfShootingMeteor4pc(), planarset = ForgeOfTheKalpagniLantern(),
                                    **config)
    ...
    TrailblazerRotationFirefly += [TrailblazerCharacter.useSuperBreak(character=FireflyCharacter, 
                                                                      baseGauge=FireflyCharacter.useEnhancedSkill().gauge) * numEnhancedFirefly]

    numBasicTrailblazer = 2.0
    numSkillTrailblazer = 1.0
    TrailblazerRotation = [ # 130 max energy
            ...
            TrailblazerCharacter.useSuperBreak(character=TrailblazerCharacter, 
                                               baseGauge=TrailblazerCharacter.useBasic().gauge) * numBasicTrailblazer,
            TrailblazerCharacter.useSuperBreak(character=TrailblazerCharacter,
                                               baseGauge=TrailblazerCharacter.useSkill().gauge) * numSkillTrailblazer,
    ]
    ...
    TrailblazerRotationRuanMei = [
                       TrailblazerCharacter.useSuperBreak(character=RuanMeiCharacter,
                                                          baseGauge=RuanMeiCharacter.useBasic().gauge) * numBasicRuanMei
    ]
    ...
    TrailblazerRotationGallagher = [
        TrailblazerCharacter.useSuperBreak(character=GallagherCharacter,
                                                             baseGauge=GallagherCharacter.useBasic().gauge) * numBasicGallagher,
        TrailblazerCharacter.useSuperBreak(character=GallagherCharacter,
                                                             baseGauge=GallagherCharacter.useEnhancedBasic().gauge) * numEnhancedGallagher,
        TrailblazerCharacter.useSuperBreak(character=GallagherCharacter,
    ...
    TrailblazerEstimate = DefaultEstimator(f'Trailblazer: {numSkillTrailblazer:.0f}E {numBasicTrailblazer:.0f}N Q', TrailblazerRotation, TrailblazerCharacter, config)
