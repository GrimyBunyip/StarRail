...
def JingyuanTingyunTopazLuocha(config):
    ...
    JingYuanCharacter = JingYuan(RelicStats(mainstats = ['ATK.percent', 'SPD.flat', 'CR', 'DMG.lightning'],
                            ...
                            relicsetone = GrandDuke2pc(), relicsettwo = GrandDuke4pc(followupStacks=4.9,uptime=1.0), planarset = FirmamentFrontlineGlamoth(stack=2),
                            **config)
    ...
    JingYuanCharacter.addStat('SPD.percent',description='Tingyun E1',amount=0.20,uptime=1.0/2.0)
    JingYuanCharacter.addStat('ATK.percent',description='Benediction',
                            amount=0.55 if TingyunCharacter.eidolon >= 5 else 0.50)
    JingYuanCharacter.addStat('DMG',description='Tingyun Ult',amount=0.65 if TingyunCharacter.eidolon >= 3 else 0.6) # full uptime on slow Jing Yuan
