...
def JingyuanTingyunHanabiLuocha(config):
    ...
    JingYuanCharacter = JingYuan(RelicStats(mainstats = ['ATK.percent', 'ATK.percent', 'CR', 'DMG.lightning'],
                            substats = {'CD': 12, 'CR': 8, 'ATK.percent': 5, 'BreakEffect': 3}),
    ...
    for character in [JingYuanCharacter, HanabiCharacter]:
    ...
    HanabiRotation = [HanabiCharacter.useBasic() * 1.5,
                    HanabiCharacter.useSkill() * 1.5,
                    HanabiCharacter.useUltimate() * 1,]
    ...
    HanabiEstimate = DefaultEstimator(f'Hanabi: 1.5N 1.5E 1Q, S{HanabiCharacter.lightcone.superposition:d} {HanabiCharacter.lightcone.name}', 
