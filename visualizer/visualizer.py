...
def visualize(visInfoList:list, visualizerPath:str='visualizer\\visual.png',  **config):
    ...
    for visInfo in visInfoList: #visInfo for each rotation
        ...
        assert abs(sum(teamValue)*cycles - teamEffect.damage) < 0.0000001, f'mismatching total effect damage and bar length: {sum(teamValue)*cycles} {teamEffect.damage}'
    ...
    for bar, teamChars, rotationName, totalEffect in zip(bars, characters, rotationNames, totalEffects):
        ...
        for i, char in enumerate(teamChars):
            ...
            picture_spacing = bar.get_x() * 0.03
            ab = AnnotationBbox(img, (bar.get_width() + bar.get_x() - picture_spacing * (len(teamChars) - i - 1), bar.get_y() + bar.get_height()/2), frameon=False)
            ax.add_artist(ab)
