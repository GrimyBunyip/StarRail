...
class Kafka(BaseCharacter):
  def __init__(self,
    ...
    self.motionValueDict['dot'] = [BaseMV(type=['dot','ultimate'],area='all', stat='atk', value=2.90, eidolonThreshold=5, eidolonBonus=0.2828)]
  ...
  def useTalent(self):
    ...
    retval.damage = self.applyDamageMultipliers(retval.damage)
    retval.gauge = 30.0 * (1.0 + self.breakEfficiency)
    retval.energy = ( 10.0 + self.bonusEnergyType['talent'] ) * ( 1.0 + self.ER )
  ...
  def useDot(self):
    ...
    retval.damage = self.applyDamageMultipliers(retval.damage)
    retval.energy = ( 0.0 + self.bonusEnergyType['dot'] + (2.0 if self.eidolon >= 4 else 0.0) ) * ( 1.0 + self.ER )
