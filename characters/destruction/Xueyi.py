...
class Xueyi(BaseCharacter):
    ...
    def __init__(self,
        ...
        # Eidolons
        if self.eidolon >= 1:
            self.addStat('DMG',description='e1',amount=0.40,type=['talent'])
        if self.eidolon >= 4:
            self.addStat('DMG',description='e4',amount=0.40,uptime=e4Uptime)
    ...
    def useTalent(self):
        ...
        retval.energy = ( 2.0 + self.getBonusEnergyAttack(type) ) * self.getER(type)
        self.addDebugInfo(retval,type)
        return retval